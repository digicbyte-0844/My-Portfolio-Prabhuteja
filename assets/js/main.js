...
(function ($) {
  ...
  // Close offcanvas when a navigation link is clicked
  $(".tw-main-menu-mobile a").on("click", function () {
    $(".tw-offcanvas-2-area").removeClass("opened");
    $(".body-overlay").removeClass("opened");
  });
  ...
  ////////////////////////////////////////////////////
  // 07. AOS Js (Optimized 60fps Performance)
  AOS.init({
    once: true,
    offset: 30,
    duration: 700,
  ...
  ////////////////////////////////////////////////////
  // 15. Contact Form Interactive Endpoint Handler
  $("form.footer-three-form, .footer-form-wrapper form, .footer-three-form").on("submit", function (e) {
    e.preventDefault();
    const $form = $(this);
    const name = $form.find('input[name="name"]').val().trim();
    const email = $form.find('input[name="email"]').val().trim();
    const userSubject = $form.find('input[name="subject"]').val() ? $form.find('input[name="subject"]').val().trim() : ("Portfolio Inquiry from " + name);
    const message = $form.find('textarea[name="message"]').val().trim();

    if (!name || !email || !message) return;

    const subject = encodeURIComponent(userSubject);
    const body = encodeURIComponent("Hello Prabhuteja,\n\nName: " + name + "\nEmail: " + email + "\n\nMessage:\n" + message);
    window.location.href = "mailto:prabhutejakoda@gmail.com?subject=" + subject + "&body=" + body;

    let $alert = $form.find(".form-feedback-alert");
    if (!$alert.length) {
      $alert = $('<div class="form-feedback-alert tw-mt-4 tw-p-4 tw-rounded-lg text-white text-center fw-medium bg-main-two-600"></div>');
      $form.append($alert);
    }
    $alert.html('<i class="ph-bold ph-check-circle me-2"></i> Thank you, ' + name + '! Your message draft is ready.');
    $alert.fadeIn();
    setTimeout(() => {
      $form[0].reset();
      $alert.fadeOut();
